...
public class DepositService {
    ...
    @Transactional
    public void makeDeposit(DepositRequestDTO depositRequestDTO){
        ...
        if (depositRequestDTO.getBillId() != null){
            BillResponseDTO bill = accountServiceClient.getBillById(depositRequestDTO.getBillId());
            bill.setAmount(bill.getAmount().add(depositRequestDTO.getAmount()));
            accountServiceClient.updateBill(depositRequestDTO.getBillId(), new BillRequestDTO(bill));
            saveDepositAndSendRabbitMQMessage(depositRequestDTO, bill);
        }
        if (depositRequestDTO.getAccountId() != null){
            BillResponseDTO defaultBill = accountServiceClient.getBillsByAccountId(depositRequestDTO.getAccountId())
                    .stream().filter(BillResponseDTO::getIsDefault).findAny().get();
    ...
    private void saveDepositAndSendRabbitMQMessage(DepositRequestDTO depositRequestDTO, BillResponseDTO defaultBill) {
        ...
        try {
            rabbitTemplate.convertAndSend(TOPIC_EXCHANGE, ROUTING_KEY, objectMapper.writeValueAsString(
                    new DepositResponseDTO(defaultBill.getAccount().getEmail(), deposit.getAmount())));
